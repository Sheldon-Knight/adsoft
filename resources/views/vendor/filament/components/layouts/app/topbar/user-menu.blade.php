...
<div x-data="{
    ...
    <button x-on:click="$refs.panel.toggle" class="block flex-shrink-0">
        <x-filament::user-avatar :user="$user" />
    </button>

    <div x-ref="panel" x-float.placement.bottom-end.offset="{ offset: 8 }" x-transition:enter="transition"
        ...
        <ul @class([
            ...
            <x-filament::dropdown.item :color="$accountItem?->getColor() ?? 'secondary'" :icon="$accountItem?->getIcon() ?? 'heroicon-s-user-circle'" :href="$accountItem?->getUrl()" tag="a">
            ...
            @foreach ($items as $key => $item)
                @if ($key !== 'account' && $key !== 'logout')
                    <x-filament::dropdown.item :color="$item->getColor() ?? 'secondary'" :icon="$item->getIcon()" :href="$item->getUrl()" tag="a">
                        {{ $item->getLabel() }}
                    </x-filament::dropdown.item>
                @endif
            @endforeach

            <x-filament::dropdown.item :color="$logoutItem?->getColor() ?? 'secondary'" :icon="$logoutItem?->getIcon() ?? 'heroicon-s-logout'" :action="$logoutItem?->getUrl() ?? route('filament.auth.logout')" method="post"
                tag="form">
